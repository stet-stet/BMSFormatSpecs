...
Since creating BMS-type files manually is an exceeding arduous task, BMS editing programs were also devised. We will call them *editor*s.
...
That, with how backward-compatibility must be valued, is a source of major headache, at least for me.
 ...
 * The character encoding is not specified in any way. 
  ...
  * The only character set safe for all implementations is ASCII.
   * eg. Running a bms with a kanji or hiragana name on Lunatic Rave 2 in a Korean Windows PC results in some characters appearing in `?`
